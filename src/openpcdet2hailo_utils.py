...
def get_pc_samples(file_names_path, source_folder_npy, destination_folder):
    # Read the list of numbers from the txt file
    with open(file_names_path, 'r') as file:
        numbers = file.read().splitlines()

    # Define the destination directories
    pc_samples_path = os.path.join(destination_folder, 'pc_samples')
    testing_path = os.path.join(pc_samples_path, 'testing')
    innoviz_path = os.path.join(testing_path, 'innoviz')

    # Create the destination directories if they do not exist
    os.makedirs(innoviz_path, exist_ok=True)

    for number in numbers:
        # Process npy files
        npy_filename = f"{number}.npy"
        source_npy_path = os.path.join(source_folder_npy, npy_filename)
        dest_npy_path = os.path.join(innoviz_path, npy_filename)

        if os.path.exists(source_npy_path):
            # Load the .npy file
            data = np.load(source_npy_path)
            # Save the data to a new .npy file at destination folder
            np.save(dest_npy_path, data)
        else:
            print(f"Warning: {npy_filename} does not exist in {source_folder_npy}")
